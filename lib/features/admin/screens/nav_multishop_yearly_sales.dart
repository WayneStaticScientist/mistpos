...
import 'package:mistpos/core/utils/pdfdocuments/pdf_multishop_yearly_sales.dart';

class NavMultiShopYearlySales extends StatefulWidget {
  final String title;
  const NavMultiShopYearlySales({
    super.key,
    this.title = 'MultiShop Yearly Sales',
  });

  @override
  State<NavMultiShopYearlySales> createState() =>
      _NavMultiShopYearlySalesState();
}

class _NavMultiShopYearlySalesState extends State<NavMultiShopYearlySales> {
  ...
  void _reloadYearlySales() {
    _multiShopController.fetchMultiShopYearlySales(date: _date);
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Obx(() {
      if (_multiShopController.loadingYearlySales.value) {
          ...
          children: [
            ...
            Row(
              children: [
                ...
                GestureDetector(
                  onTap: () async {
                    if (_multiShopController.multishopYearlySales.isEmpty) {
                      Toaster.showError("No data to print");
                      return;
                    }
                    try {
                      final pdf = await MultiShopYearlySalesPdf.generate(
                        date: _date,
                        baseCurrence:
                            _userController.user.value?.baseCurrence ?? '',
                        multishopData:
                            _multiShopController.multishopYearlySales,
                      );
            ...
            _buildSectionLabel('Monthly Analytics'),
        ...
        return Container(
          ...
          child: Column(
            ...
            children: [
              ...
              const Divider(),
              Column(
                children: _multiShopController.multishopYearlySales.map((
                  shop,
                ) {
  ...
  Widget _buildChartLegend() {
    return Wrap(
      spacing: 12,
      runSpacing: 8,
      children: _multiShopController.multishopYearlySales.asMap().entries.map((
        entry,
      ) {
        return Row(
          mainAxisSize: MainAxisSize.min,
  ...
  Widget _buildMultilineChart(
    ...
    // months from 1 to 12
    List<int> allMonths = List.generate(12, (index) => index + 1);

    final lineBarsData = _multiShopController.multishopYearlySales
        .asMap()
        .entries
        .map((entry) {
          ...
          final List<dynamic> monthlyDataList = shop.dailyData['monthlyData'] ?? [];

          final spots = allMonths.map((month) {
            final point = monthlyDataList.firstWhere(
              (p) => p['month'] == month,
              orElse: () => <String, dynamic>{},
            );

            final value = point[dataKey] ?? 0.0;
            return FlSpot(month.toDouble(), value.toDouble());
          }).toList();
            ...
            touchTooltipData: LineTouchTooltipData(
              ...
              getTooltipItems: (touchedSpots) {
                return touchedSpots.map((spot) {
                  ...
                  final formattedMonth = _formatMonthLabel(i);
                  ...
                  final shopName = _multiShopController
                      .multishopYearlySales[shopIndex]
                      .companyName;
