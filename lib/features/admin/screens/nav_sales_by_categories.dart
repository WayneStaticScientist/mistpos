...
import 'package:mistpos/core/utils/pdfdocuments/pdf_sales_by_categories.dart';

class NavSalesByCategory extends StatefulWidget {
  const NavSalesByCategory({super.key});

  @override
  State<NavSalesByCategory> createState() => NavSalesByCategoryState();
}

class NavSalesByCategoryState extends State<NavSalesByCategory> {
  ...
  void _loadData() {
    _adminController.getSalesByCategory(_startDate, _endDate);
      ...
      child: Column(
        ...
        children: [
          ...
          Wrap(
            ...
            runSpacing: 16,
            children: [
              const Text("Category Sales & Performance", style: TextStyle(fontSize: 24, fontWeight: FontWeight.bold)),
              Row(
          ...
          Obx(() {
            if (_adminController.loadingSalesByCategory.value) {
              return const SizedBox(height: 300, child: Center(child: MistLoader1()));
            }
            if (_adminController.salesByCategory.isEmpty) {
              return SizedBox(
                height: 300,
                child: Center(
            ...
            final totalGross = _adminController.salesByCategory.fold(0.0, (prev, data) => prev + data.grossSales);
            final totalItemsSold = _adminController.salesByCategory.fold(0, (prev, data) => prev + data.itemsSold);
            ...
            return Column(
              ...
              children: [
                ...
                LayoutBuilder(
                  builder: (ctx, constraints) {
                    ...
                    return GridView.count(
                      ...
                      children: [
                        _buildKpiCard("Active Categories", _adminController.salesByCategory.length.toDouble(), Colors.purple.shade400, Icons.category, isCurrency: false),
                        _buildKpiCard("Gross Sales", totalGross, primary, Icons.monetization_on, isCurrency: true),
                        _buildKpiCard("Items Sold", totalItemsSold.toDouble(), Colors.orange.shade500, Icons.shopping_cart, isCurrency: false),
                      ],
                    );
                  },
                ),
                ...
                LayoutBuilder(
                  builder: (ctx, constraints) {
                    ...
                    return isWide
                        ? Row(
                            children: [
                              Expanded(child: _buildPieChartCard("Sales by Category")),
                            ],
                          )
                        : Column(
                            children: [
                              _buildPieChartCard("Sales by Category"),
                            ],
                          );
                  },
                ),
                ...
                // ── DATA TABLE ──
                Text("Category Sales Data", style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Theme.of(context).colorScheme.onSurface)),
                const SizedBox(height: 16),
                SizedBox(
                  height: 400,
                  child: _makeTable(_adminController.salesByCategory),
                ),
              ],
            );
          }),
  ...
  Widget _buildPieChartCard(String title) {
    final list = _adminController.salesByCategory;
    final validData = list.where((e) => e.grossSales > 0).toList();
    validData.sort((a, b) => b.grossSales.compareTo(a.grossSales));
    
    final topData = validData.take(5).toList();
    final otherTotal = validData.skip(5).fold(0.0, (prev, e) => prev + e.grossSales);
                ...
                Expanded(
                  child: SingleChildScrollView(
                    child: Column(
                      ...
                      children: [
                        for (int i = 0; i < topData.length; i++) ...[
                          _buildIndicator(colors[i], topData[i].categoryName),
  ...
  Widget _makeTable(RxList<SalesByCategoryModel> salesByCategory) {
    final isDark = Theme.of(context).brightness == Brightness.dark;
    final baseCurrence = _userController.user.value?.baseCurrence ?? '';
    return Container(
      decoration: BoxDecoration(
      ...
      child: DataTable2(
        ...
        columns: [
          DataColumn2(label: Text('Category', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.grey.shade600)), size: ColumnSize.L),
          ...
          DataColumn2(label: Text('Items Sold', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.grey.shade600)), size: ColumnSize.S, numeric: true),
        ],
        rows: salesByCategory.map((e) {
          return DataRow(
            cells: <DataCell>[
              ...
              DataCell(Text(e.itemsSold.toString())),
            ],
          );
        }).toList(),
      ),
  ...
  void printDocument() async {
    ...
    try {
      final pdf = await PdfSalesByCategories.generate(
        endDate: _endDate,
        startDate: _startDate,
        baseCurrence: baseCurrency,
        salesByCategories: _adminController.salesByCategory,
      );
