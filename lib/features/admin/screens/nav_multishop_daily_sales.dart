...
import 'package:mistpos/core/utils/pdfdocuments/pdf_multishop_daily_sales.dart';

class NavMultiShopDailySales extends StatefulWidget {
  final String title;
  const NavMultiShopDailySales({
    super.key,
    this.title = 'MultiShop Daily Sales',
  });

  @override
  State<NavMultiShopDailySales> createState() => _NavMultiShopDailySalesState();
}

class _NavMultiShopDailySalesState extends State<NavMultiShopDailySales> {
  ...
  void _reloadDailySales() {
    _multiShopController.fetchMultiShopDailySales(date: _date);
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Obx(() {
      if (_multiShopController.loadingDailySales.value) {
          ...
          children: [
            ...
            Row(
              children: [
                    ...
                    child: Container(
                      ...
                      child: Row(
                        children: [
                          ...
                          Text(
                            MistDateUtils.getInformalDate(_date),
                ...
                GestureDetector(
                  onTap: () async {
                    if (_multiShopController.multishopDailySales.isEmpty) {
                      Toaster.showError(
                        "No data to print",
                      );
                      return;
                    ...
                    try {
                      final pdf = await MultiShopDailySalesPdf.generate(
                        date: _date,
                        baseCurrence:
                            _userController.user.value?.baseCurrence ?? '',
                        multishopData: _multiShopController.multishopDailySales,
                      );
            ...
            _buildSectionLabel('Hourly Analytics'),
        ...
        return Container(
          ...
          child: Column(
            ...
            children: [
              ...
              const Divider(),
              Column(
                children: _multiShopController.multishopDailySales.map((shop) {
  ...
  Widget _buildChartLegend() {
    return Wrap(
      spacing: 12,
      runSpacing: 8,
      children: _multiShopController.multishopDailySales.asMap().entries.map((
        entry,
      ) {
        return Row(
          mainAxisSize: MainAxisSize.min,
  ...
  Widget _buildMultilineChart(
    ...
    final lineBarsData = _multiShopController.multishopDailySales
        .asMap()
        .entries
        .map((entry) {
          ...
          final spots = allHours.map((hour) {
            ...
            return FlSpot(hour.toDouble(), value.toDouble());
          }).toList();
            ...
            touchTooltipData: LineTouchTooltipData(
              ...
              getTooltipItems: (touchedSpots) {
                return touchedSpots.map((spot) {
                  ...
                  final formattedHour = _formatHourLabel(i);
