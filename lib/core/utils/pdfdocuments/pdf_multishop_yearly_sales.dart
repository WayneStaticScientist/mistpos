...
class MultiShopYearlySalesPdf {
          ...
          return [
              ...
              children: [
                pw.Column(
                  ...
                  children: [
                    ...
                    pw.Text(
                      "MultiShop Yearly Sales Overview",
                      style: pw.TextStyle(
                        fontSize: 18,
            ...
            pw.Row(
              children: [
                ...
                pw.Text(yearStr, style: const pw.TextStyle(fontSize: 12)),
              ],
            ),
            ...
            if (multishopData.isNotEmpty)
              _buildMonthlyGraphTable(multishopData, baseCurrence, date)
            else
              pw.Text(
  ...
  static pw.Widget _buildStatsTable(
    List<MultiShopDailySalesData> shops,
    String baseCurrence,
  ) {
  ...
  static pw.Widget _buildMonthlyGraphTable(
    List<MultiShopDailySalesData> shops,
    String baseCurrence,
    DateTime date,
  ) {
    // Only show months where there is activity across any shop to save space
    Set<int> activeMonths = {};
    for (var shop in shops) {
      final List<dynamic> monthlyData = shop.dailyData['monthlyData'] ?? [];
      for (var point in monthlyData) {
        if ((point['revenue'] ?? 0) > 0 ||
            (point['expenses'] ?? 0) > 0 ||
            (point['receipts'] ?? 0) > 0) {
          activeMonths.add(point['month']);
        }
      }
    }

    // If no active months, just show everything or an empty state
    ...
    List<int> displayMonths = activeMonths.toList()..sort();
    ...
    return pw.Table(
      ...
      children: [
        ...
        ...displayMonths.map((month) {
          ...
          final monthStr = monthsShort[month - 1];
          return pw.TableRow(
            children: [
              pw.Padding(
                padding: const pw.EdgeInsets.all(8),
              ...
              ...shops.map((shop) {
                ...
                final formatted =
                    "${CurrenceConverter.getCurrenceFloatInStrings(revenue, baseCurrence)} / ${CurrenceConverter.getCurrenceFloatInStrings(expenses, baseCurrence)}";
